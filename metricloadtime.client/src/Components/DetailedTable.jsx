...
const Example = ({combinations, initialcombinations, setinitialcombinations, genereatedLoadTimes, setGenereatedLoadTimes, rowSelection, setRowSelection, thresholdValue, filteredData, setFilteredData}) => {
  ...
  // Polling function to fetch data from the API
  const fetchData = async () => {
    try {
      const response = await fetch('api/adomd/getloadtime', { method: 'GET' });
      if (response.ok) {
        const results = await response.json();
        setinitialcombinations(results);
  ...
  useEffect(() => {
    const intervalId = setInterval(async () => {
      const loadTimes = initialcombinations.map(entry => entry.loadTime);
      if (!loadTimes.includes("x")) {
        clearInterval(intervalId); // Stop polling if all load times are fetched

        // Fetch one more time after interval ends
        await fetchData();
      } else {
        await fetchData();
      }
  ...
  }, [initialcombinations, setinitialcombinations, setFilteredData]);

  const columns = useMemo(() => [
    { accessorKey: 'measure', header: 'Measure' },
    { accessorKey: 'dimensionName', header: 'Dimension Name' },
    { accessorKey: 'columnName', header: 'Column Name' },
    {
      accessorKey: 'loadTime', header: 'Load Time', Cell: ({ cell }) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {cell.getValue() === 'x' ? <CircularProgress className="loadtimeprogress" /> : <span>{cell.getValue()}</span>}
        </Box>
      ),
    },
    { accessorKey: 'previousLoadTime', header: 'Previous Load Time' },
    { accessorKey: 'reportName', header: 'Report Name' },
    { accessorKey: 'pageName', header: 'Page Name' },
    { accessorKey: 'visualName', header: 'Visual Name' },
    { accessorKey: 'visualTitle', header: 'Visual Title' },
  ], []);
